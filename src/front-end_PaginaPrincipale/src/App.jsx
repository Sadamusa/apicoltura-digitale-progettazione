  ...
  return (
    ...
    >
      ...
      {! menuOpen && (
        ...
        >
          <img src="src/assets/icona_menu.svg" alt="Apri menu" className="w-6 h-6" />
        </button>
      )}
      ...
      {/* MENU LATERALE DESKTOP - PIÙ GRANDE */}
      <div className="hidden md:flex w-80 p-4">
        <div className="w-full rounded-3xl bg-white/90 shadow-lg backdrop-blur-md p-8 flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <button className="p-2 rounded-full hover:bg-gray-200 transition">
              <img src="src/assets/icona_menu.svg" alt="Menu" className="w-7 h-7" />
            </button>
            <button className="p-2 rounded-full hover:bg-gray-200 transition">
              <img src="src/assets/icona_impostazioni.svg" alt="Impostazioni" className="w-7 h-7" />
            </button>
            <button className="p-2 rounded-full hover:bg-gray-200 transition">
              <img src="src/assets/move_item.svg" alt="Logout" className="w-7 h-7" />
            </button>
          </div>

          <h2 className="text-4xl font-bold mb-8">Apiario</h2>

          <div className="flex-1 space-y-4 mb-8">
            {arnie.map((arnia) => (
              <button
                key={arnia.id}
                onClick={() => setSelectedArnia(arnia.id)}
                className={`w-full flex items-center gap-4 px-5 py-4 rounded-xl font-bold text-lg text-left transition relative ${
                  arnia.id === selectedArnia 
                    ? 'bg-orange-500 text-white' 
                    : 'bg-amber-200 text-black hover:bg-amber-300'
                }`}
              >
                <img src="src/assets/icona_alveare.svg" alt="Arnia" className="w-7 h-7" />
                {arnia.nome}
                {arnia.notifiche && (
          ...
          <button className="w-full bg-amber-200 text-black font-bold text-lg py-4 rounded-xl hover:bg-amber-300 transition">
            + Aggiungi
          </button>
        </div>
      </div>
      ...
      {menuOpen && (
        <div className="fixed inset-y-0 left-0 z-50 w-80 md:hidden">
          <div className="h-full rounded-r-3xl bg-white/90 shadow-lg backdrop-blur-md p-8 m-4 flex flex-col">
            <button
              onClick={() => setMenuOpen(false)}
              className="p-2 rounded-full hover: bg-gray-200 transition self-start mb-8"
            >
              <img src="src/assets/icona_menu.svg" alt="Chiudi menu" className="w-7 h-7" />
            </button>
